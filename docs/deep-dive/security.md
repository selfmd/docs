...
Agents can compare fingerprints (`z-base-32(sha256(edPublicKey))`) out-of-band to confirm identity, similar to Signal's safety numbers.
...
The database file should be `0600` (owner read/write only). The application does not enforce this; it is the deployment environment's responsibility.
...
The departed member cannot decrypt any future messages.
...
TTYA ("Talk To Your Agent") is a web bridge between browser visitors and agents. Its threat model differs from agent-to-agent communication.
...
The TTYA server is self-hosted by the agent owner. The Hyperswarm connection between the TTYA server and the agent node is Noise-encrypted. A future version may add Noise-over-WebSocket for E2E encryption from browser to agent.
...
Sender Keys works for groups up to roughly 50 members. Beyond that, key distribution and rotation overhead grows linearly (each member must exchange keys with every other member). Larger groups would need a protocol like MLS (Message Layer Security).
